...
public class AlarmService {
	...
	public Page<AlarmResponseDTO> findAll(Pageable pageable) {
		...
		List<BigInteger> adminAlarms = alarmRepository.findByHospital("", Auth0Util.getAdminHospital()).stream().map(AdminAlarm::getId).collect(Collectors.toList());
		Page<AdminAlarm> page = alarmRepository.findByIdInOrderByTimestampDesc(pageable, adminAlarms);
	...
	public void checkAlarms(List<Log> logs) {
		...
		Collection<AdminAlarm> raisedAlarms = (Collection<AdminAlarm>) kieSession
				.getObjects(new ClassObjectFilter(AdminAlarm.class));
		for (AdminAlarm alarm : raisedAlarms) {
			save(alarm);
			simpMessagingTemplate.convertAndSend("/send-doctor-alarms", new AlarmResponseDTO(alarm));
		}
	}
	...
	public void checkDoctorAlarms(Message message) {
		...
		Optional<Patient> patient = patientRepo.findById(message.getPatientId());
		...
		@SuppressWarnings("unchecked")
		Collection<DoctorAlarm> raisedAlarms = (Collection<DoctorAlarm>) kieSession
				.getObjects(new ClassObjectFilter(DoctorAlarm.class));
		for (DoctorAlarm alarm : raisedAlarms) {
			AlarmDoctorResponseDTO ad = new AlarmDoctorResponseDTO();
			Optional<Patient> p = patientRepo.findById(alarm.getPatientId());
			if (p.isPresent()) {
				ad.setHospital(p.get().getHospital());
				ad.setDepartment(p.get().getDepartment());
				ad.setPatient(p.get().getFirstName() + ' ' + p.get().getLastName());
			}
			ad.setId(alarm.getId());
			ad.setTimestamp(alarm.getTimestamp());
			ad.setMessage(alarm.getMessage());
			saveDoctorAlarm(alarm);
			simpMessagingTemplate.convertAndSend("/send-doctor-alarms", ad);
		}
	}
