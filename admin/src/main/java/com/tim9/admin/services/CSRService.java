...
public class CSRService {
	...
	/*
	 * Izvori: https://smallstep.com/blog/everything-pki/
	 * https://www.bouncycastle.org/docs/pkixdocs1.5on/org/bouncycastle/pkcs/
	 * PKCS10CertificationRequest.html
	 * https://www.programcreek.com/java-api-examples/?api=org.bouncycastle.asn1.
	 * x509.Extension https://tools.ietf.org/html/rfc3279
	 * 
	 */
	...
	public CSR saveCSR(byte[] csr) throws IOException, InvalidKeyException, OperatorCreationException,
			NoSuchAlgorithmException, PKCSException, InvalidDigitalSignatureException {
		...
		String hospital = subject.getRDNs(BCStyle.O)[0].getFirst().getValue().toString();
		if (this.hospitalRepository.findByName(hospital) == null)
			this.hospitalRepository.save(new Hospital(null, subject.getRDNs(BCStyle.O)[0].getFirst().getValue().toString(),
	...
	private boolean isValidSigned(PKCS10CertificationRequest csr)
			throws InvalidKeyException, OperatorCreationException, NoSuchAlgorithmException, PKCSException {
		BouncyCastleProvider bc = new BouncyCastleProvider();
		ContentVerifierProvider provider = new JcaContentVerifierProviderBuilder().setProvider(bc)
	...
	public Page<CSRResponseDTO> findAll(Pageable pageable) throws IOException {
		...
		for (CSR csr : page.getContent()) {
			...
			X500Name subject = req.getSubject();
			CSRResponseDTO dto = new CSRResponseDTO(csr.getId(),
					...
					subject.getRDNs(BCStyle.L)[0].getFirst().getValue().toString(),
	...
	public String rejectCSR(Long serialNumber) throws IOException {
		...
		if (csr.isPresent()) {
			...
			X500Name subject = req.getSubject();
			emailService.sendMail(subject.getRDNs(BCStyle.E)[0].getFirst().getValue().toString(), "CSR Rejection",
	...
	public CSRResponseDTO getCSR(Long serialNumber) throws IOException {
		...
		if (csr.isPresent()) {
			JcaPKCS10CertificationRequest req = new JcaPKCS10CertificationRequest(csr.get().getCsr());
			X500Name subject = req.getSubject();
			CSRResponseDTO dto = new CSRResponseDTO(csr.get().getId(),
					subject.getRDNs(BCStyle.CN)[0].getFirst().getValue().toString(),
					subject.getRDNs(BCStyle.O)[0].getFirst().getValue().toString(),
					subject.getRDNs(BCStyle.OU)[0].getFirst().getValue().toString(),
					subject.getRDNs(BCStyle.L)[0].getFirst().getValue().toString(),
	...
	public String createNonCACertificate(CertDataDTO dto) throws Exception {
		...
		X509v3CertificateBuilder certGen = new JcaX509v3CertificateBuilder(issuerName,
				new BigInteger(dto.getSerialNumber()), dto.getNotBefore(), dto.getNotAfter(), subject,
				csrData.getPublicKey());
